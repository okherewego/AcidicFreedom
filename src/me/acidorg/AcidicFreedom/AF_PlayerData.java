...
public class AF_PlayerData
{
    public final static Map<Player, AF_PlayerData> userinfo = new HashMap<Player, AF_PlayerData>();
    ...
    public AF_PlayerData(Player player)
    {
        this.player = player;
    ...
    public static AF_PlayerData getPlayerData(Player p)
    {
        AF_PlayerData playerdata = AF_PlayerData.userinfo.get(p);

        if (playerdata == null)
        {
            Iterator<Entry<Player, AF_PlayerData>> it = userinfo.entrySet().iterator();
            while (it.hasNext())
            {
                Entry<Player, AF_PlayerData> pair = it.next();
                AF_PlayerData playerdata_test = pair.getValue();
        ...
        if (playerdata == null)
        {
            playerdata = new AF_PlayerData(p);
            AF_PlayerData.userinfo.put(p, playerdata);
        }
    ...
    public void startArrowShooter(AcidicFreedom plugin)
    {
