...
public class AF_SuperadminList
{
    private static Map<String, AF_Superadmin> superadminList = new HashMap<String, AF_Superadmin>();
    ...
    public static void loadSuperadminList()
    {
        try
        {
            ...
            AF_Util.createDefaultConfiguration(AcidicFreedom.SUPERADMIN_FILE, AcidicFreedom.plugin_file);
            FileConfiguration config = YamlConfiguration.loadConfiguration(new File(AcidicFreedom.plugin.getDataFolder(), AcidicFreedom.SUPERADMIN_FILE));
            ...
            if (config.isConfigurationSection("superadmins"))
            {
                ...
                {
                    AF_Superadmin superadmin = new AF_Superadmin(admin_name, section.getConfigurationSection(admin_name));
                    superadminList.put(admin_name.toLowerCase(), superadmin);
                }
            }
            else
            {
                AF_Log.warning("Missing superadmins section in superadmin.yml.");
            }
    ...
    {
        File a = new File(AcidicFreedom.plugin.getDataFolder(), AcidicFreedom.SUPERADMIN_FILE);
        File b = new File(AcidicFreedom.plugin.getDataFolder(), AcidicFreedom.SUPERADMIN_FILE + ".bak");
        FileUtil.copy(a, b);
    }
    ...
    public static void updateIndexLists()
    {
        ...
        Iterator<Entry<String, AF_Superadmin>> it = superadminList.entrySet().iterator();
        while (it.hasNext())
        {
            Entry<String, AF_Superadmin> pair = it.next();

            String admin_name = pair.getKey().toLowerCase();
            AF_Superadmin superadmin = pair.getValue();
        ...
        superadminNames = AF_Util.removeDuplicates(superadminNames);
        superadminIPs = AF_Util.removeDuplicates(superadminIPs);
        seniorAdminNames = AF_Util.removeDuplicates(seniorAdminNames);
    }
    ...
    public static void saveSuperadminList()
    {
        try
        {
            ...
            while (it.hasNext())
            {
                ...
                config.set("superadmins." + admin_name + ".ips", AF_Util.removeDuplicates(superadmin.getIps()));
                config.set("superadmins." + admin_name + ".last_login", AF_Util.dateToString(superadmin.getLastLogin()));
                ...
                config.set("superadmins." + admin_name + ".console_aliases", AF_Util.removeDuplicates(superadmin.getConsoleAliases()));
                config.set("superadmins." + admin_name + ".is_activated", superadmin.isActivated());
            }

            config.save(new File(AcidicFreedom.plugin.getDataFolder(), AcidicFreedom.SUPERADMIN_FILE));
        }
        catch (Exception ex)
        {
            AF_Log.severe(ex);
    ...
    public static AF_Superadmin getAdminEntry(String admin_name)
    {
    ...
    public static AF_Superadmin getAdminEntry(Player p)
    {
        return getAdminEntry(p.getName().toLowerCase());
    }

    public static AF_Superadmin getAdminEntryByIP(String ip)
    {
        Iterator<Entry<String, AF_Superadmin>> it = superadminList.entrySet().iterator();
        while (it.hasNext())
        {
    ...
    {
        AF_Superadmin admin_entry = getAdminEntry(p);
        if (admin_entry != null)
        {
    ...
    public static boolean isSeniorAdmin(CommandSender user, boolean verify_is_superadmin)
    {
        ...
        AF_Superadmin admin_entry = getAdminEntry((Player) user);
        if (admin_entry != null)
        {
    ...
    public static boolean checkPartialSuperadminIP(String user_ip, String user_name)
    {
        try
        {
            ...
            else
            {
                ...
                {
                    if (AF_Util.fuzzyIpMatch(user_ip, test_ip, 3))
                    {
                        match_ip = test_ip;
                        break;
                    }
                ...
                if (match_ip != null)
                {
                    AF_Superadmin admin_entry = getAdminEntryByIP(match_ip);
    ...
    public static void addSuperadmin(String admin_name, List<String> ips)
    {
        try
        {
            ...
            if (superadminList.containsKey(admin_name))
            {
                AF_Superadmin superadmin = superadminList.get(admin_name);
            ...
            else
            {
                ...
                AF_Superadmin superadmin = new AF_Superadmin(admin_name, ips, last_login, custom_login_message, is_senior_admin, console_aliases, true);
                superadminList.put(admin_name.toLowerCase(), superadmin);
            }
            ...
            while (it.hasNext())
            {
                ...
                if (superadmin.isActivated() && !superadmin.isSeniorAdmin())
                {
                    ...
                    if (hours_since_login > clean_threshold_hours)
                    {
                        if (verbose)
                        {
                            AF_Util.adminAction("TotalFreedomSystem", "Deactivating superadmin \"" + superadmin.getName() + "\", inactive for " + hours_since_login + " hours.", true);
                        }
    ...
    public static boolean verifyIdentity(String admin_name, String ip) throws Exception
    {
        ...
        AF_Superadmin admin_entry = getAdminEntry(admin_name);
        if (admin_entry != null)
        {
